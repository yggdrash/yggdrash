...
public class DPoAContractTest {
    ...
    public void setUp() throws IllegalAccessException {
        ...
        List<Field> txReceipt = ContractUtils.txReceiptFields(dPoAContract);
        if (txReceipt.size() == 1) {
        ...
        for (Field f : ContractUtils.contractFields(dPoAContract, ContractStateStore.class)) {
            f.setAccessible(true);
    ...
    public void voteValidatorFailTxValidation() throws Exception {
        ...
        TransactionReceipt receipt = dPoAContract.voteValidator(JsonUtil.parseJsonObject("{}"));
        assertEquals(ExecuteStatus.ERROR, receipt.getStatus());
    }
    ...
    public void commitDisagreeValidator() throws Exception {
        ...
        proposeValidatorSet = dPoAContract.getProposeValidatorSet();
        assertEquals(1, proposeValidatorSet.getValidatorMap().size());
